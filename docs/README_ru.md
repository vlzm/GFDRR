...
```bash
uv venv
uv pip install -e ".[dev,ui,api]"  # симулятор + веб-интерфейс + API + инструменты разработки
...
- **`app/`** — то, что запускается. `app/runner.py` выполняет один сценарий
  ...
  уже посчитал сборщик артефактов. `app/ui_shared.py` держит общие помощники
  страниц; `app/backend.py` — единственное место, где приложение выбирает
  между локальными файлами и HTTP-запросами через `app/api_client.py` (когда
  установлена переменная `API_URL`), и для чтения запусков, и для их
  старта. `app/api.py` — API артефактов запусков: HTTP-сервис, который
