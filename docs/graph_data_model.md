...
**ResourceCategory (L2)** — тип ресурса. Определяет базовые характеристики: capacity, compatible commodities, compatible modal types, cost profile. По нему индексируется transport_cost.
...
Параметрические таблицы (`operation_cost`, `transport_cost`) **остаются с `date`** в сырых данных. При сборке модели добавляется шаг **time resolution** — маппинг дат на period_id с агрегацией.
...
| transport_cost  | edge × resource_category × date  | да           | COST       | Стоимость перевозки                   |
...
| transport_cost  | edge × resource_category × date | edge × resource_category × period_id | COST       |
...
resource_cost                    # кастомные costs/rates для ресурсов (через Attribute System)
├── resource_category (FK)
...
`resource_cost` — generic таблица для произвольных resource attributes с `kind=COST` или `kind=RATE`. Каждый `attribute_name` — отдельный атрибут, зарегистрированный через AttributeSpec. Пользователь определяет сколько угодно кастомных costs.

**Design note: EAV trade-off.** `resource_cost` использует Entity-Attribute-Value паттерн (`attribute_name` как колонка), тогда как `operation_cost` и `transport_cost` — explicit columns. Это сознательный выбор: набор resource costs полностью domain-specific и непредсказуем (fuel, maintenance, insurance, license_fee, compliance_cost...), тогда как operation_cost и transport_cost имеют стабильную структуру. EAV trade-offs: (+) произвольное количество кастомных атрибутов без schema migration, (+) единый механизм регистрации через AttributeSpec; (−) сложнее валидация допустимых `attribute_name`, (−) нет database-level type safety, (−) pivot-операции при запросах. Валидация допустимых `attribute_name` обеспечивается через регистрацию в AttributeSpec — незарегистрированные атрибуты отклоняются на этапе Validation (§13.2).
...
Aggregation parameters в `scenario` определяют, на каком уровне иерархии агрегировать модель (§7.4). `planning_horizon_id` — FK на planning_horizon, позволяет переиспользовать один горизонт между сценариями.

### 11.12. Output tables — выходные данные потребителей

Определяют grain выходных данных потребителей (optimizer, simulator, etc.). Эти таблицы **генерируются** потребителем, не вводятся вручную. Grain совпадает с planning model для прямого сравнения с Historical layer (§12). Подробнее о разных потребителях — §15.
...
Raw data (с date, lead_time_hours)
    ↓
Validation (unit consistency, referential integrity, completeness)
    ↓
Time Resolution (date → period_id, агрегация)
    ↓
Lead Time Resolution (lead_time_hours → edge_lead_time_resolved)
    ↓
Resolved data (с period_id, lead_time_periods per edge×period)
    ↓
Edge Building (rules + manual + commodity filtering + modal_type)
    ↓
Transformation Resolution (какие facility конвертируют что, N→M)
    ↓
Fleet Capacity Computation (resource_fleet.count → effective capacity)
    ↓
[Optional] Hierarchical Aggregation (facility и/или commodity, §7.4)
    ↓
Grain Grouping + Spine Assembly
    ↓
...
**Unit consistency** — `demand.quantity_unit` совпадает с `commodity_category.unit` для соответствующих category. То же для supply, inventory_initial, edge capacities.
...
**Transformation consistency** — для каждой трансформации input commodities доступны на incoming edges facility, output commodities — на outgoing edges.

```python
class ValidationResult:
    errors: list[ValidationError]      # блокируют сборку
...
def build_resolved_model(
    raw_params: dict[str, DataFrame],   # "operation_cost" → df с date
    periods: DataFrame,                  # period_id, start_date, end_date
...
### 13.8. [Optional] Шаг Hierarchical Aggregation

Если в `scenario` заданы `facility_hierarchy_type` / `commodity_hierarchy_type`, модель агрегируется (§7.4). Уровень агрегации задаётся в конфигурации сценария.
