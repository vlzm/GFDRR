...
slice_a = ui_shared.panel_slice(panel_a, period, commodity)
data = facilities.merge(slice_a, on="facility_id", how="left")
if not show_depots:
...
if run_b:
    ...
    b_columns = [f"{name}_b" for name in PANEL_VALUES]
    data[b_columns] = data[b_columns].fillna(0)

if view == VIEW_A:
    data["value"] = data[metric]
elif view == VIEW_B:
    data["value"] = data[f"{metric}_b"]
else:
...
def _tooltip_html(row: pd.Series) -> str:
    ...
    for name in PANEL_VALUES:
        ...
        if run_b:
            value_b = int(row[f"{name}_b"])
            lines.append(
                f"<tr><td>{label}</td><td>{value_a}</td><td>{value_b}</td>"
                f"<td>{value_b - value_a:+d}</td></tr>"
            )
        else:
...
with st.expander("Data table"):
    ...
    if run_b:
        shown += [f"{name}_b" for name in PANEL_VALUES]
