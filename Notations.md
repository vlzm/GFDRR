...
| `seasonal naive` | The baseline model, and the first model of the platform: the forecast for a station at a given hour is the mean demand at the same hour of week over the historical weeks (`seasonal_naive_demand`). Every later model must beat it. | `naive baseline` (as a data name) |
