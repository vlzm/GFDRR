...
| `forecast run` | A sized run whose demand table is a forecast demand table (§17) instead of a historical one. The demand is first cut to what the scenario can run (`restrict_demand_to_scenario`); the dropped share goes into `meta.json` (`forecast_dropped_share`), and the run's `meta.json` records the forecast's name. |
