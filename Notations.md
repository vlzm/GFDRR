...
| `forecast run` | A sized run whose demand table is a forecast demand table (§17) instead of a historical one. A forecast can name stations or station-hours the scenario's trip CSV has never seen; those rows have no OD rows to run on, so the demand is first cut to what the scenario can run (`restrict_demand_to_scenario`, the same cut the two-level evaluation uses) and the dropped share goes into `meta.json` (`forecast_dropped_share`). After the cut the run goes through the same path — `size_state_for_demand` sizes the state, `run_sized_scenario` runs it — and its `meta.json` records the name of the forecast it used. |
...
| `month period grid` | The hourly period grid of one calendar month: `period_id` 0, 1, 2, … from the month's first hour, with each hour's wall-clock bounds (`month_period_grid` in `gbp/ml/data.py`). One grid for every month-shaped task: a training partition numbers its hours on it, and a forecast of a held-out month uses it as its forecast horizon (the backtest, the monitoring baseline, the two-level evaluation). | `month grid`, `hourly grid of the month` |
...
| `seasonal naive` | The baseline model, and the first model of the platform: the forecast for a station at a given hour is the mean demand at the same hour of week over the history window. Since phase 4 it lives behind the model interface (`SeasonalNaiveModel` in `gbp/ml/models/seasonal_naive.py`) and reads the feature column `facility_hour_of_week_mean` — the same number, computed once by the feature module. Every later model must beat it. Its forecast of one calendar month is built by one function, `naive_month_prediction` in `gbp/ml/forecast.py`, and both quality measures read it: the backtest's over-naive columns and the monitoring alert's `naive_mae`. | `naive baseline` (as a data name) |
...
| `MLflow store` | The one folder MLflow keeps everything in (default `data/ml/mlflow/`): the experiment runs and the model registry in `mlflow.db` (SQLite), the logged files under `artifacts/`. In code it is the object `MlflowStore` (`gbp/ml/registry.py`): an entry point creates it once and passes it on, every registry operation is a method on it, and each method first points MLflow's process-wide state at the folder — no caller has to remember a "configure first" rule. | `tracking dir` (as a concept name), `mlflow backend` |
