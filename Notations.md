...
| `data/ml/` | The forecasting data (§17): training tables in `training/`, one folder per saved forecast in `forecasts/` (the forecast demand table plus its `meta.json`), forecast-quality metrics in `monitoring/`, the local MLflow store in `mlflow/` (experiment runs and the model registry), and the pipeline log `pipeline_log.csv` (§17). Created in the demand forecasting phase. | "model folder", "ml artifacts" |
