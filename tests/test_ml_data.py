"""Tests for the data step (phase 2 of the demand forecasting plan).
...
def test_normalize_month_accepts_both_forms():
    assert data.normalize_month("202502") == "202502"
    assert data.normalize_month("2025-02") == "202502"
...
def test_month_csvs_matches_city_and_jersey_city_files(raw):
    ...
    found = data.month_csvs("2026-01", raw)
...
def test_old_schema_maps_to_the_trips_schema(raw):
    ...
    trips = data.load_trips_any_schema(str(path))
...
def test_new_schema_goes_through_the_raw_loader_unchanged(raw):
    ...
    via_any = data.load_trips_any_schema(str(path))
...
def test_old_schema_load_is_cached_in_processed(raw, tmp_path):
    path = write_csv(raw, "201906-citibike-tripdata_1.csv", [OLD_HEADER, old_row()])
    first = data.load_trips_any_schema(str(path))
    assert (tmp_path / "processed" / "201906-citibike-tripdata_1.parquet").exists()
    again = data.load_trips_any_schema(str(path))
...
def test_month_zip_keys_picks_city_and_jersey_city(monkeypatch):
    ...
    monkeypatch.setattr(data, "_list_bucket_keys", lambda prefix: listing.get(prefix, []))
    assert data.month_zip_keys("202601") == [
...
def test_month_zip_keys_names_the_yearly_bundle_when_monthly_is_gone(monkeypatch):
    monkeypatch.setattr(data, "_list_bucket_keys", lambda prefix: [])
    with pytest.raises(ValueError, match="2019-citibike-tripdata.zip"):
        data.month_zip_keys("201906")
...
def test_download_months_skips_a_month_already_on_disk(raw, monkeypatch):
    ...
    monkeypatch.setattr(data, "_list_bucket_keys", no_network)
    monkeypatch.setattr(data, "_download", no_network)
    assert data.download_months(["202601"], raw) == []
...
def test_download_months_extracts_only_flat_csvs(raw, monkeypatch):
    ...
    monkeypatch.setattr(data, "_download", lambda url, dest: dest.write_bytes(payload.getvalue()))
    extracted = data.download_months(["202401"], raw)
