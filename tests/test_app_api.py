"""Tests of the run-artifact API (``app/api.py``).

Described in ``docs/site/content/docs/reference/api.md``.
...
def test_table_cache_key_is_a_constant_over_http(monkeypatch):
    # Served artifacts are immutable
    # (docs/site/content/docs/reference/api.md): the HTTP cache key must
